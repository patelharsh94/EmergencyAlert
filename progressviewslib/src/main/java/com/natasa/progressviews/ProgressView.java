...
abstract class ProgressView extends View implements ProgressShape {
     ...
     */
    public void setProgress(float progress, float prev_progress) {
        setProgressInView(progress, prev_progress);
    }

    private void setProgressInView(float progress, float prev_progress) {
        this.progress = (progress <= maximum_progress) ? progress : maximum_progress;
        initAnimator(progress, prev_progress);
        invalidate();
...
// ***********************ANIMATOR**********************************************
    ...
    // }
    /*
    * changed the function and all the functions that call this method, to have
    * a previous progress, so we can gradually increase the progress, for smoother animation.*/
    private void initAnimator(float progress, float prev_progress) {
        objAnimator = ObjectAnimator.ofFloat(this, PROGRESS,prev_progress, progress);
    ...
    public void setProgressIndeterminateAnimation(int animSpeedMillisec) {
        ...
        initAnimator(maximum_progress, 0);
        objAnimator.setDuration(animSpeedMillisec);
